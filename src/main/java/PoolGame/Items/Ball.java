...
public class Ball implements Drawable, Movable {
    ...
    public Ball() {
        ...
        this.mouseImage = new Image("/club.png");
        this.mouseImageView = new ImageView(mouseImage);
        this.mouseImageView.setVisible(false);
        this.mouseImageView.setFitHeight(imageSize);
        this.mouseImageView.setFitWidth(imageSize);
        this.mouseImageView.setPreserveRatio(true);

        rotate = new Rotate();
        // 设置旋转中心（可选）
        rotate.setPivotX(this.mouseImageView.getFitWidth() / 2);
        rotate.setPivotY(this.mouseImageView.getFitHeight() / 2);
        this.mouseImageView.getTransforms().add(rotate);
    }
    ...
    public void addToGroup(ObservableList<Node> groupChildren) {
        ...
        groupChildren.add(this.mouseImageView);
    }
    ...
    private double calAngle(double x1, double y1, double x2, double y2) {
        ...
        double angle = (float) (Math.acos(x / z) / Math.PI * 180) ;
        if (y2 < y1) {
            angle = 360 - angle;
        }
        return 180 - angle;
    ...
    private void registerCueBallMouseAction() {
        this.shape.setOnMouseDragged(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    this.mouseImageView.setVisible(true);
                    rotate.setAngle(45 + calAngle(actionEvent.getSceneX(), actionEvent.getSceneY(),
                             this.shape.getCenterX(), this.shape.getCenterY()));
                    this.mouseImageView.setLayoutX(actionEvent.getSceneX() - this.mouseImageView.getFitWidth() / 2);
                    this.mouseImageView.setLayoutY(actionEvent.getSceneY() - this.mouseImageView.getFitHeight() / 2);
/*
        ...
        this.shape.setOnMouseReleased(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    ...
                    // Logging status for rollback
                    hit_num ++;
                    System.out.println(hit_num+"ball hit num");
                }
            }
        );
    }
